...
def Randwalk(p,times,length,window,min_count,alpha):
    ...
    np.savetxt('/home/hongqiaochen/Desktop/Link_predict/Power/Power_walkpath.txt', walkpath,fmt='%d',delimiter=' ')
    walkpath = word2vec.Text8Corpus('/home/hongqiaochen/Desktop/Link_predict/Power/Power_walkpath.txt')
    model = word2vec.Word2Vec(walkpath,size = 64,hs = 1,min_count = min_count,window = window,sg=1,alpha=alpha)
    model.wv.save_word2vec_format('/home/hongqiaochen/Desktop/Link_predict/Power/Power_vector.txt')
    V = np.loadtxt('/home/hongqiaochen/Desktop/Link_predict/Power/Power_vector.txt', dtype=float,skiprows=1)
